        ...
        <PageTransition>
            <div className="p-8 md:p-10 min-h-full relative font-sans overflow-y-auto custom-scrollbar">
                ...
                <div className="relative z-10 max-w-7xl mx-auto space-y-8">
                    ...
                    {/* Header */}
                    <LiquidGlassCard className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 p-8 relative overflow-hidden" interactive={false} colorHint="bg-blue-400">
                        <div className="absolute -left-20 -top-20 w-64 h-64 bg-blue-100/50 rounded-full blur-3xl opacity-50 pointer-events-none" />
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="flex flex-wrap items-center gap-4 text-sm font-semibold text-gray-500">
                                <span className="flex items-center gap-1.5 bg-blue-500/10 text-blue-700 px-3 py-1 rounded-full border border-blue-500/20"><Cpu size={14} /> System Online</span>
                                <span className="flex items-center gap-1.5 bg-emerald-500/10 text-emerald-700 px-3 py-1 rounded-full border border-emerald-500/20"><Database size={14} /> DB Connected</span>
                                <span className="text-gray-400">v2.0.4.beta</span>
                            </div>
                        </div>

                        <button className="flex items-center gap-2 font-bold text-sm px-6 py-3 rounded-xl border border-blue-200 bg-white/40 backdrop-blur-md hover:bg-white/60 transition-all text-blue-800 shadow-sm active:scale-95 relative z-10">
                        ...
                        {stats.map((stat, idx) => {
                            ...
                            return (
                                <LiquidGlassCard key={stat.label}
                                    className="relative p-6 flex items-center justify-between group overflow-hidden"
                                    interactive={true} colorHint="bg-blue-300"
                                >
                                    <div className={`absolute -right-6 -top-6 w-24 h-24 rounded-full ${stat.bg} blur-2xl opacity-50 group-hover:opacity-100 transition-opacity pointer-events-none`} />
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        {/* Security Monitor */}
                        <LiquidGlassCard className="lg:col-span-2 flex flex-col relative h-[450px] overflow-hidden group" interactive={false} colorHint="bg-emerald-300">
                        ...
                        <LiquidGlassCard className="flex flex-col h-[450px] overflow-hidden" interactive={false} colorHint="bg-fuchsia-300">
                            ...
                            <div className="p-4 bg-gray-50/20 border-t border-white/40 backdrop-blur-sm">
                                <button
                                    onClick={() => alert("Loading all past recordings is currently disabled.")}
                                    className="w-full py-3 rounded-xl font-bold text-sm text-fuchsia-700 bg-white/50 border border-fuchsia-200 hover:bg-white/70 hover:border-fuchsia-300 transition-all shadow-sm active:scale-95"
                                >
                                    Load All Records
                                </button>
                            </div>
